...
class ChatGPTAPI: LLMClient, @unchecked Sendable {
  ...
  init(
    apiKey: String, model: String = "gpt-3.5-turbo",
    systemPrompt: String = "You are a helpful assistant", temperature: Double = 0.5
  ) {
    self.apiKey = apiKey
    self.model = model
  ...
  func sendMessageStream(text: String) async throws -> AsyncThrowingStream<String, Error> {
    ...
    guard 200...299 ~= httpResponse.statusCode else {
      ...
      if let data = errorText.data(using: .utf8),
        let errorResponse = try? jsonDecoder.decode(ErrorRootResponse.self, from: data).error
      {
